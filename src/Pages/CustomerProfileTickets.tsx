...
const formatTicketAge = (value: unknown) => {
  ...
  const totalMinutes = Math.floor(
    Math.max(Date.now() - parsed.getTime(), 0) / 60000,
  );
...
const CustomerProfileTickets = ({
  customerId,
  customerName,
}: CustomerProfileTicketsProps) => {
  ...
  const ticketsQuery = useCustomerWiseAllTicketList(
    payload,
    Boolean(customerId),
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    const selectedStatus = statusOptions
      ...
      ?.label.toLowerCase();
    const selectedAgent = agentOptions
      .find((option) => option.value === agent)
      ?.label.toLowerCase();

    return rows.filter((row: unknown) => {
      const ticketStatus = text(
        getValue(row, ["cTicketStatus", "Status", "StatusName"]),
        "",
      ).toLowerCase();
      const ticketAgent = text(
        getValue(row, ["cAgentName", "AgentName", "AssignedAgent"]),
        "",
      ).toLowerCase();
      const ticketActivity = text(
        getValue(row, ["cRepairItemActivity", "cActivityName", "ActivityName"]),
        "",
      ).toLowerCase();
      const searchable = [
  ...
  const openFollowUp = (row: unknown) => {
    const ticketId = Number(
      getValue(row, ["nTicketId", "TicketId", "nTicketid"]) || 0,
    );
    if (!ticketId) return;
  ...
  const filterPanel = (
    <div className="flex h-[430px] w-[335px] flex-col">
      <h2 className="m-0 border-b px-3 pb-3 text-lg font-medium">
        Apply Filter
      </h2>
      ...
      <div className="flex flex-none justify-end gap-4 px-3 pt-2">
        <Button
          className="h-10 min-w-[83px] border-emerald-500 text-emerald-500"
    ...
    <section className="flex h-full min-h-0 min-w-0 flex-1 flex-col gap-3 overflow-hidden p-5">
      <div className="flex flex-none flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-3">
          ...
          <Popover
            ...
            open={filterOpen}
            onOpenChange={(open) =>
              open ? openFilters() : setFilterOpen(false)
            }
          >
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden border-b">
        ...
        {ticketsQuery.isFetching ? (
          ...
          visibleRows.map((row: unknown, index: number) => {
            const ticketId = Number(
              getValue(row, ["nTicketId", "TicketId", "nTicketid"]) || 0,
            );
            const createdDate = getValue(row, [
              "dCreatedDate",
              "CreatedDate",
              "CreatedDateTime",
              "dCreatedOn",
            ...
            return (
              ...
              >
                ...
                <span className="space-y-1 pr-3">
                  ...
                  {activitySummary ? (
                    <span className="block text-[10px] leading-4 text-blue-600">
                      {activitySummary}
                    </span>
                  ) : null}
                </span>
                <span>
                  {text(getValue(row, ["nTicketNo", "TicketNo", "cTicketNo"]))}
                </span>
                <span className="pr-3">
                  {text(
                    getValue(row, [
                ...
                <span>
                  {text(
                    getValue(row, ["cTicketStatus", "StatusName", "Status"]),
                  )}
                </span>
              </button>
            );
