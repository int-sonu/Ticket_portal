...
import type { SharedAgentOption } from "../Common/AgentSelectorModal";
...
type PunchAction = "punch-in" | "punch-out";
...
const unwrap = (response: any): any => {
  ...
  for (let index = 0; index < 6; index += 1) {
    ...
    const nested =
      ...
      value.AttendanceSummary ??
      value.attendanceSummaryMonthly ??
      value.AttendanceSummaryMonthly ??
      value.attendanceSummayMonthly ??
      value.AttendanceSummayMonthly ??
      value.attendanceSummaryMonthlyList ??
      value.AttendanceSummaryMonthlyList ??
...
const rowsFrom = (response: any): RecordLike[] => {
  ...
  while (queue.length) {
    ...
    for (const nested of Object.values(current as RecordLike)) {
      if (Array.isArray(nested)) {
        const records = nested.filter(
          (item): item is RecordLike => Boolean(item) && typeof item === "object",
        );
        if (records.length) return records;
        if (nested.length === 0) foundEmptyArray = true;
...
const extractPunchinId = (response: unknown) => {
  ...
  if (typeof unwrapped === "number" || /^\d+$/.test(String(unwrapped ?? ""))) {
  ...
  while (queue.length) {
    ...
    for (const [key, nested] of Object.entries(value as RecordLike)) {
      const isAttendanceRecordId =
        /(?:punch|attendance).*id|id.*(?:punch|attendance)/i.test(key) &&
        !/(?:agent|company|user|created|modified)/i.test(key);
      const candidate = isAttendanceRecordId ? Number(nested) || 0 : 0;
...
const hasRecordedTime = (value: unknown) => {
...
const attendanceRowDateKey = (row: RecordLike, month: dayjs.Dayjs) => {
  ...
  const resolvedDate = dateKey(dateValue);
  if (resolvedDate) return resolvedDate;

  const dayFromLabel = String(dateValue ?? "").trim().match(/^(\d{1,2})(?:\s|$)/)?.[1];
  const day = Number(
    getValue(row, ["nDay", "Day", "day", "nDate", "DateNo"], dayFromLabel ?? 0),
  );
...
const PunchInOutPage = () => {
  ...
  const punchCompletedStorageKey = `attendance-punched-out:${basePayload.nCompanyId ?? 0}:${currentAgentId}:${dayjs().format("YYYY-MM-DD")}`;
  const [savedPunchinId, setSavedPunchinId] = useState(
    () => Number(sessionStorage.getItem(punchIdStorageKey)) || 0,
  );
  const [completedAction, setCompletedAction] = useState<PunchAction | null>(
    () =>
      sessionStorage.getItem(punchCompletedStorageKey) === "1"
        ? "punch-out"
        : null,
  );
  ...
  const agentOptions = useMemo<SharedAgentOption[]>(() => {
    const rows = rowsFrom(agentQuery.data);
    ...
    }).filter((agent: SharedAgentOption) => agent.label && agent.value);
  }, [agentQuery.data]);
  ...
  const punchIn = getValue(status, ["cPunchInTime", "cPunchinTime", "dPunchInTime", "dPunchinTime", "cInTime", "cCheckInTime", "PunchInTime", "punchIn"]);
  const punchOut = getValue(status, ["cPunchOutTime", "cPunchoutTime", "dPunchOutTime", "dPunchoutTime", "cOutTime", "dOutTime", "cCheckOutTime", "dCheckOutTime", "PunchOutTime", "OutTime", "punchOut"]);
  ...
  const dailyPunchIn = getValue(daily, ["cPunchInTime", "cPunchinTime", "dPunchInTime", "cInTime", "cCheckInTime", "PunchInTime", "punchIn"], punchIn);
  const dailyPunchOut = getValue(daily, ["cPunchOutTime", "cPunchoutTime", "dPunchOutTime", "cOutTime", "dOutTime", "cCheckOutTime", "dCheckOutTime", "PunchOutTime", "OutTime", "punchOut"], punchOut);
  const rawStatus = String(
    getValue(status, ["cStatus", "AttendanceStatus", "attendanceStatus", "status", "nStatus", "nAttendanceStatus"], ""),
  ).trim().toLowerCase();
  const punchOutFlag = String(
    getValue(status, ["bPunchOut", "bIsPunchOut", "isPunchOut", "IsPunchOut", "bPunchedOut"], ""),
  ).trim().toLowerCase();
  const punchInFlag = String(
    getValue(status, ["bPunchIn", "bIsPunchIn", "isPunchIn", "IsPunchIn", "bPunchedIn"], ""),
  ).trim().toLowerCase();
  const serverPunchedOut =
    hasRecordedTime(punchOut) ||
    hasRecordedTime(dailyPunchOut) ||
    ["1", "true", "yes"].includes(punchOutFlag) ||
    ["2", "out", "punch out", "punched out", "already punched out"].includes(rawStatus);
  const serverPunchedIn =
    !serverPunchedOut &&
    (hasRecordedTime(punchIn) ||
      hasRecordedTime(dailyPunchIn) ||
      ["1", "true", "yes"].includes(punchInFlag) ||
      ["1", "in", "punch in", "punched in"].includes(rawStatus));
  const isPunchedOut =
    completedAction === "punch-out" ||
    (completedAction !== "punch-in" && serverPunchedOut);
  const isPunchedIn =
    !isPunchedOut && (completedAction === "punch-in" || serverPunchedIn);
  ...
  const punchMutation = useMutation({
    mutationFn: async (action: PunchAction) => {
      if (action === "punch-in") {
        return attendanceApis.punchIn(punchActionPayload);
      }

      let activePunchinId = punchinId;
      if (!activePunchinId) {
        const latestStatus = await attendanceApis.attendanceStatus(
          attendanceStatusPayload,
        );
        activePunchinId = extractPunchinId(latestStatus);
        if (activePunchinId) {
          sessionStorage.setItem(punchIdStorageKey, String(activePunchinId));
          setSavedPunchinId(activePunchinId);
        }
      }

      return attendanceApis.punchOut({
        nPunchinId: activePunchinId,
        ...punchActionPayload,
      });
    },
    onSuccess: async (response, action) => {
      ...
      if (action === "punch-out") {
        sessionStorage.setItem(punchCompletedStorageKey, "1");
        sessionStorage.removeItem(punchIdStorageKey);
        setSavedPunchinId(0);
      } else {
        sessionStorage.removeItem(punchCompletedStorageKey);
      ...
      message.success(action === "punch-out" ? "Punch out recorded." : "Punch in recorded.");
    },
    onError: (error: any) => {
          ...
          <div className="flex min-h-[590px] flex-col border-l-[5px] border-r border-l-sky-800 border-r-slate-200 px-4 py-3">
            ...
            {isPunchedOut ? (
              ...
              <button
                type="button"
                onClick={() =>
                  punchMutation.mutate(isPunchedIn ? "punch-out" : "punch-in")
                }
            ...
            <div className="max-h-[430px] overflow-y-auto overflow-x-hidden">
              {monthlyDays.map((date) => {
                const row = monthlyRows.find(
                  (item) =>
                    attendanceRowDateKey(item, summaryMonth) ===
                    date.format("YYYY-MM-DD"),
                ) ?? {};
                const rowPunchIn = getValue(row, [
                  "cPunchInTime",
                  "cPunchinTime",
                  "dPunchInTime",
                  "dPunchinTime",
                ...
                const rowPunchOut = getValue(row, [
                  "cPunchOutTime",
                  "cPunchoutTime",
                  "dPunchOutTime",
                  "dPunchoutTime",
                ...
                const rowDuration = Math.max(
                  durationSeconds(rowWorking),
                  Number.isFinite(workingMinutes) ? workingMinutes * 60 : 0,
                  elapsedSeconds(rowPunchIn, rowPunchOut),
                );
                return (
